...
statement : ID EQ exprr {assign_value($1, $3);}
          ...
          | ID PLSEQ exprr {assign_number_with_operator($1, $3, $2);}
          | ID MINEQ exprr {assign_number_with_operator($1, $3, $2);}
          | ID MULEQ exprr {assign_number_with_operator($1, $3, $2);}
          | ID DIVEQ exprr {assign_number_with_operator($1, $3, $2);}
          | function_call
          | PRINT POPEN exprr PCLOSE {Adaugare($3);}
          | statement COMMA statement
          | 
          ;

exprr : exprr PLS exprr {$$ = $1 + $3;}
     | exprr MIN exprr {$$ = $1 - $3;}
     | exprr MUL exprr {$$ = $1 * $3;}
     | exprr DIV exprr {$$ = $1 / $3;}
     | POPEN exprr PCLOSE {$$ = $2;}
     | NUMBER {$$ = $1;}
...
call_list : exprr {$$ = malloc (256); strcpy($$, "int, ");}
